import { GatewayIntentBits, Client, messageLink } from 'discord.js';
...
client.on('ready', function () {
    ...
    try {
        ...
        client.allowedUsers = JSON.parse(data)["allowedUsers"];
...
const handleCommand = async (client, message) => {
    ...
    if (!client.allowedUsers.includes(message.author.id)) {
        return message.channel.send('You dont have access to this bot, request it from Chresh.');
    }
    ...
    if (commandName == 'ai' || commandName == 'aih') {
        ...
        try {
            ...
            console.log(prompt);
            /*const response = await axios.post("http://127.0.0.1:11000/api/chat", {
                model: "qwen3:30b",
                messages: [
                    {
                        role: "system",
                ...
                transformResponse: [function (data) {
                    ...
                    console.log("SOD" + data + "EOD")

                }]
            }*///);

            /*for await (const chunk of await response.data) {
                console.log(chunk)
            }*/
